...
from NezuNotify.nezu_puls import NezuPuls
...
def token_management_example():
    print("=== トークン管理の例 ===")
    nezu_create = NezuPuls(csrf=csrf, cookie=cookie, target_mid=target_mid)
    ...
    # トークンのステータス確認
    nezu_check = NezuPuls(token=new_token)
    status = nezu_check.process("check", new_token)
    print(f"トークンのステータス: {status}")

    # トークンの無効化
    nezu_revoke = NezuPuls(token=new_token)
